...
from abebe.core.config import *
from abebe.ui.fake_hack import start_fake_hack
from abebe.ui.good_end import show_good_end
from abebe.ui.minigame_pinball import start_pinball
from abebe.core.background_music import play_music, stop_music, resume_music
from abebe.ui.hack_decoder import show_hack_decoder
from abebe.ui.abebe_confirm_exit import show_abebe_confirm
from abebe.core.game_state import exit_game_confirmed
from abebe.ui.window_registry import register, unregister
...
from abebe.core.game_launcher import run_pygame_session
from abebe.ui.level_editor_window import show_level_editor
from abebe.custom_maps import CustomMapError, load_custom_map_document
...
def show_help_window(root):
    ...
    commands = [
        ...
        "run edit - open level editor",
        "!tut - open tutor / training maze",
        "!city - open city maze",
        "load test - open testing maze",
        "load custom <map> - open player map",
    ]
...
def show_iobey_audio(root):
    ...
    audio_path = os.path.join(get_exe_dir(), "data", "app", "story", "iobey98.wav")
...
def show_gallery_window(root):
    ...
    data_dir = os.path.join(get_exe_dir(), "data", "app", "story")
...
def show_password_window(root):
    ...
    def check():
        ...
        run_target = None
        if pwd.startswith("run ") and len(pwd) > 4:
            run_target = pwd[4:].strip()

        if " " in pwd and load_target is None and run_target is None:
            entry_var.set("")
            set_input_enabled(True)
            return
        ...
        if load_target is not None:
            validation_text = f"load{load_target.replace(' ', '')}"
        elif run_target is not None:
            validation_text = f"run{run_target}"
        else:
            validation_text = pwd
        ...
        if pwd.startswith("!"):
            if pwd == "!help":
                ...
                append_terminal_lines([f"> {pwd}"])
                info_path = os.path.join(get_exe_dir(), "data", "app", "story", "info.txt")
                ...
                tk.Label(dev_content, text="Watcher systems: disabled", fg="lime", bg="black", font=("Consolas", 12)).pack(padx=10, pady=5)
            elif pwd == "!reset":
                append_terminal_lines([f"> {pwd}"])
                password_history.clear()
            elif pwd == "!hack":
                append_terminal_lines([f"> {pwd}"])
                show_hack_decoder(root, "UNKNOWN")
                ...
                append_terminal_lines([f"> {pwd}"])
                incident_path = os.path.join(get_exe_dir(), "data", "app", "story", "12340.txt")
                ...
                append_terminal_lines([f"> {pwd}"])
                egg_path = os.path.join(get_exe_dir(), "data", "app", "story", "easteregg.png")
        ...
        if load_target is not None:
            if load_target.startswith("custom "):
                custom_map_name = load_target[7:].strip()
                if not custom_map_name:
                    lines = [f"> load {load_target}", "Custom map name is missing.", ""]
                    entry_var.set("")
                    type_terminal_lines(lines, on_complete=lambda: set_input_enabled(True))
                    return

                try:
                    document = load_custom_map_document(custom_map_name)
                except CustomMapError as exc:
                    lines = [f"> load {load_target}", str(exc), ""]
                    entry_var.set("")
                    type_terminal_lines(lines, on_complete=lambda: set_input_enabled(True))
                    return
                ...
                entry_var.set("")
                type_loading_command(
                    f"custom {resolved_name}",
                    on_complete=lambda: run_pygame_session(root, start_custom_maze, root, resolved_name),
                )
                return
