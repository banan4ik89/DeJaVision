...
from abebe.events.creeper_event import CreeperEvent
# ===================== РЎРћРЎРўРћРЇРќРРЇ =====================
...
# ===================== Р¤РђР™Р›Р« GIF =====================
GIFS = {
    STATE_NEUTRAL: "app/abebe/abebe_neutral.gif",
    STATE_HAPPY: "app/abebe/abebe_happy.gif",
    STATE_ANGRY: "app/abebe/abebe_angry.gif"
}
...
# ===================== РЈРўРР›РРўРђ РџР•Р Р•РўРђРЎРљРР’РђРќРРЇ =====================
...
class AbebeWatcher:
    ...
    # ===================== РћРўР”Р•Р›Р¬РќРћР• РћРљРќРћ РўР•РљРЎРўРђ =====================
    ...
    # ===================== РћР‘РќРћР’Р›Р•РќРР• РЎРћРЎРўРћРЇРќРРЇ =====================
    ...
    # РёРЅРёС†РёР°Р»РёР·Р°С†РёСЏ С‚РµРј
        if not hasattr(self, "topics"):
            self.setup_topics()
            self.show_dialog(f"Topic selected: {self.current_topic['name']}")

    # С…СЂР°РЅРёР»РёС‰Рµ РёСЃРїРѕР»СЊР·РѕРІР°РЅРЅС‹С… СЃР»РѕРІ
    ...
    # ----------- РЈР›РЈР§РЁР•РќРќРђРЇ Р›РћР“РРљРђ РЎРћР’РџРђР”Р•РќРР™ -----------

        def normalize(word):
        # РјРЅРѕР¶РµСЃС‚РІРµРЅРЅРѕРµ С‡РёСЃР»Рѕ
        ...
        def matches(word, word_list):
            ...
            # СЃРѕРІРїР°РґРµРЅРёРµ РїРѕСЃР»Рµ РЅРѕСЂРјР°Р»РёР·Р°С†РёРё
                if word_norm == base:
                    return True
            ...
            # С‡Р°СЃС‚РёС‡РЅРѕРµ СЃРѕРІРїР°РґРµРЅРёРµ
    ...
    # ===================== РЈРџР РђР’Р›Р•РќРР• Р’РР”РРњРћРЎРўР¬Р® =====================
    def hide_for_game(self):
        """РЎРєСЂС‹С‚СЊ РѕРєРЅР° РІРѕ РІСЂРµРјСЏ РёРіСЂС‹"""
        if self.win.winfo_exists():
            self.win.withdraw()
        if self.text_win.winfo_exists():
    ...
    def show_after_game(self):
        """РџРѕРєР°Р·Р°С‚СЊ РѕРєРЅР° РїРѕСЃР»Рµ РёРіСЂС‹"""
        if self.win.winfo_exists():
            self.win.deiconify()
        if self.text_win.winfo_exists():
