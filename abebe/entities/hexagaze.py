...
def collect_sentries(map_data, radius_min, radius_max, orb_cycle):
    sentries = []
    if isinstance(map_data, dict) and map_data.get("sentries"):
        for index, sentry_spawn in enumerate(map_data["sentries"]):
            x = float(sentry_spawn.get("x", sentry_spawn.get("cell_x", 0) + 0.5))
            y = float(sentry_spawn.get("y", sentry_spawn.get("cell_y", 0) + 0.5))
            cell_x = int(sentry_spawn.get("cell_x", int(x)))
            cell_y = int(sentry_spawn.get("cell_y", int(y)))
            ...
            sentries.append(
                {
                    ...
                    "zone_cells": set(),
                    "visible_cells": set(),
                    "orb_cycle_index": index % max(1, len(orb_cycle)),
                }
            )
        return sentries
