...
from abebe.ui.password_window import show_password_window
from abebe.ui.intro import show_intro
from abebe.core.background_music import play_music
from abebe.core.config import BACKGROUND_MUSIC
from abebe.ui.new_game_window import show_new_game_window
from abebe.ui.settings_window import show_settings
from abebe.ui.level_editor_window import show_level_editor
from abebe.core import game_state

import time
from abebe.core.user_settings import load_settings, save_settings
...
def show_splash(root):
    save_settings({"main_menu_intro_seen": True})
    ...
    hands_dir = Path(get_exe_dir()) / "data" / "levels" / "assets" / "hands"
...
def show_placeholder_program(root, program_name, description):
    attr_name = f"_{program_name.lower().replace(' ', '_')}_window"
    existing = getattr(root, attr_name, None)
    ...
    win.geometry("420x240+240+180")
    setattr(root, attr_name, win)
    ...
    close_btn = tk.Button(
        title_bar,
        text="X",
        command=lambda: [setattr(root, attr_name, None), win.destroy()],
        bg="#c0c0c0",
        fg="black",
    ...
    body = tk.Frame(win, bg=PANE_BG, highlightbackground=PANE_BORDER_DARK, highlightthickness=2)
    body.pack(expand=True, fill="both", padx=6, pady=6)
    ...
    if cursor_api is not None:
        ...
        cursor_api["register_click_widget"](close_btn)

    def close_placeholder():
        setattr(root, attr_name, None)
        if win.winfo_exists():
            win.destroy()

    win.bind("<Escape>", lambda _e: close_placeholder())
    win.protocol("WM_DELETE_WINDOW", close_placeholder)
...
root.after(100, lambda: show_splash(root) if not load_settings().get("main_menu_intro_seen", False) else None)
root.deiconify()
...
start_menu_state = {"window": None}

def close_start_menu(_event=None):
    menu = start_menu_state["window"]
    if menu is None:
        return
...
def launch_from_start(command):
    close_start_menu()
    command()

def open_start_menu(_event=None):
    menu = start_menu_state["window"]
    ...
    menu.configure(bg=PANE_BG_DARK, highlightbackground=PANE_BORDER_DARK, highlightthickness=2)
    menu.attributes("-topmost", True)
    start_menu_state["window"] = menu

    menu_width = 320
    menu_height = 430
    x_pos = max(8, taskbar.winfo_rootx() + 8)
    y_pos = taskbar.winfo_rooty() - menu_height - 2
    menu.geometry(f"{menu_width}x{menu_height}+{x_pos}+{y_pos}")

    sidebar = tk.Frame(menu, bg=PANE_TITLE, width=64)
    ...
    programs = [
        ("ABEBE_PROTOCOL.exe", lambda: start_new_game()),
        ("save_data.bin", lambda: None),
        ("secret_files/", lambda: game_state.start_game(show_password_window)),
        ("system_settings.ini", lambda: show_settings(root)),
        ("my_computer.lnk", lambda: show_intro(root)),
        ("Brerder.exe", lambda: show_level_editor(root)),
        ("Calculator.exe", lambda: show_placeholder_program(root, "Calculator", "Temporary placeholder for calculator.")),
        ("Paint.exe", lambda: show_placeholder_program(root, "Paint", "Temporary placeholder for paint program.")),
        ("Text Editor.exe", lambda: show_placeholder_program(root, "Text Editor", "Temporary placeholder for text editor.")),
        ("shutdown.exe", lambda: exit_app()),
    ]
    ...
    for label, command in programs:
        btn = tk.Button(
            body,
            text=label,
            command=lambda cmd=command: launch_from_start(cmd),
            anchor="w",
            font=("Terminal", 12),
            bg=PANE_BG,
            fg=PANE_TEXT,
        ...
        btn.bind("<Enter>", lambda _e, widget=btn: widget.config(bg=PANE_ACTIVE))
        btn.bind("<Leave>", lambda _e, widget=btn: widget.config(bg=PANE_BG))
        root._cursor_api["register_click_widget"](btn)

    def on_focus_out(_event=None):
        root.after(120, close_start_menu)

    menu.bind("<FocusOut>", on_focus_out)
    menu.bind("<Escape>", close_start_menu)
    menu.focus_force()
...
start_placeholder.bind("<Button-1>", open_start_menu, add="+")
