...
CUSTOM_ROOM_CEILING_HEIGHT = 2.4
...
# Second height map for ceilings.
# `1` is the default tutorial ceiling, higher digits raise the room.
CEILING_MAP = [
...
# Optional upper wall layer. `#` adds one extra wall block above the base wall.
UPPER_WALL_MAP = [
    "...........................",
    "...........................",
    "...........................",
...
def _ramp_progress(x, y, rotation_degrees):
    ...
    dot_min = min(corners)
    dot_max = max(corners)
    current = local_x * dir_x + local_y * dir_y
    if abs(dot_max - dot_min) < 1e-6:
        return 0.0
    return max(0.0, min(1.0, (current - dot_min) / (dot_max - dot_min)))


def _stair_floor_height(x, y, layer_index, cell):
    base_z = layer_index * 1.0
    stair_height = cell["height"] * 1.0
    return base_z + _ramp_progress(x, y, float(cell.get("rotation", 0.0))) * stair_height


def _collision_sample_points(x, y, radius=PLAYER_COLLISION_RADIUS):
    return (
        (x, y),
        (x + radius, y),
        (x - radius, y),
...
def _stair_link_height(x, y, z_hint=None):
    geometry = CUSTOM_RUNTIME_GEOMETRY
    if geometry is None:
        return None
    best_height = None
    best_dist = None
    for link in geometry.get("stair_links", []):
        t, dist = _point_to_segment_projection(
            x,
            y,
            link["start_x"],
            link["start_y"],
        ...
        if dist > link.get("width", 0.34):
            continue
        height = link["start_z"] + (link["end_z"] - link["start_z"]) * t
        if z_hint is not None and height > z_hint + RAMP_REACH_HEIGHT + 0.08:
            continue
        if best_height is None or height > best_height or (abs(height - best_height) < 1e-6 and (best_dist is None or dist < best_dist)):
            best_height = height
            best_dist = dist
    return best_height


def get_walk_support_height(x, y, z_hint=None, radius=PLAYER_COLLISION_RADIUS):
    best_floor = get_floor_height(x, y, z_hint=z_hint)
    for sample_x, sample_y in _collision_sample_points(x, y, radius):
        sample_floor = get_floor_height(sample_x, sample_y, z_hint=z_hint)
        if sample_floor > best_floor:
            best_floor = sample_floor
    return best_floor


def can_occupy_position(x, y, z_hint, blocker_fn=None, radius=PLAYER_COLLISION_RADIUS, body_height=DEFAULT_EYE_HEIGHT):
    support_z = get_walk_support_height(x, y, z_hint=z_hint, radius=radius)
    for sample_x, sample_y in _collision_sample_points(x, y, radius):
        sample_floor = get_floor_height(sample_x, sample_y, z_hint=max(z_hint, support_z))
        # Prevent the player capsule from hanging half inside a tilted volume:
        # every support sample needs to stay reasonably close to the chosen stand height.
        # But when the player is already standing on a raised object and walks off the edge,
        # we should allow the move and let gravity take over instead of "gluing" them to it.
        if sample_floor < support_z - STEP_HEIGHT:
            if z_hint < support_z - STEP_HEIGHT * 0.5:
                return False, support_z
        ceiling_z = get_ceiling_height(sample_x, sample_y, z_hint=max(z_hint, support_z))
        if support_z + body_height >= ceiling_z - 0.01:
            return False, support_z
        collision_z = max(z_hint, sample_floor) + 0.05
        if is_wall(sample_x, sample_y, collision_z):
            return False, support_z
        if blocker_fn is not None and blocker_fn(sample_x, sample_y):
            return False, support_z
    return True, support_z
...
def _runtime_layers_at(x, y):
    ...
    cell_x = int(x)
    cell_y = int(y)
    if cell_y >= geometry["height"] or cell_x >= geometry["width"]:
        return None
    return geometry["layers"], cell_x, cell_y


def _nearby_runtime_collision_objects(x, y):
    geometry = CUSTOM_RUNTIME_GEOMETRY
    if geometry is None:
        return []
    bucket_x = int(math.floor(x))
    bucket_y = int(math.floor(y))
    return geometry.get("collision_buckets", {}).get((bucket_x, bucket_y), [])


def _inverse_rotate_runtime_point(world_x, world_y, world_z, obj):
    cx = obj["center_x"]
    cy = obj["center_y"]
    cz = obj["base_z"] + obj["height"] * 0.5
    dx = world_x - cx
    dy = world_y - cy
    dz = world_z - cz
    rz = math.radians(float(obj.get("rotation", 0.0)) % 360.0)
    ry = math.radians(float(obj.get("rotation_y", 0.0)) % 360.0)
    rx = math.radians(float(obj.get("rotation_x", 0.0)) % 360.0)
...
def _runtime_object_contains_xy(obj, world_x, world_y):
    local_x, local_y, _local_z = _inverse_rotate_runtime_point(world_x, world_y, obj["base_z"] + obj["height"] * 0.5, obj)
    half_x = 0.5 * float(obj.get("scale_x", 1.0))
    half_y = 0.5 * float(obj.get("scale_y", 1.0))
    return -half_x <= local_x <= half_x and -half_y <= local_y <= half_y


def _runtime_stair_floor_height(obj, world_x, world_y):
    local_x, local_y, _local_z = _inverse_rotate_runtime_point(world_x, world_y, obj["base_z"] + obj["height"] * 0.5, obj)
    half_x = 0.5 * float(obj.get("scale_x", 1.0))
    half_y = 0.5 * float(obj.get("scale_y", 1.0))
    if not (-half_x <= local_x <= half_x and -half_y <= local_y <= half_y):
        return None
    normalized_x = (local_x + half_x) / max(1e-6, half_x * 2.0)
    normalized_y = (local_y + half_y) / max(1e-6, half_y * 2.0)
    progress = _ramp_progress(normalized_x, normalized_y, float(obj.get("rotation", 0.0)))
    return obj["base_z"] + progress * obj["height"]


def _runtime_box_vertical_span(obj, world_x, world_y):
    if obj.get("type") not in {"wall", "box"}:
        return None
    if abs(float(obj.get("rotation_x", 0.0))) > 0.01 or abs(float(obj.get("rotation_y", 0.0))) > 0.01:
        return None
    local_x, local_y, _local_z = _inverse_rotate_runtime_point(
        world_x,
        world_y,
        obj["base_z"] + obj["height"] * 0.5,
        obj,
    )
    half_x = 0.5 * float(obj.get("scale_x", 1.0))
    half_y = 0.5 * float(obj.get("scale_y", 1.0))
    if not (-half_x <= local_x <= half_x and -half_y <= local_y <= half_y):
        return None
    return obj["base_z"], obj["base_z"] + obj["height"]


def _rotate_runtime_local_point(local_x, local_y, local_z, obj):
    rz = math.radians(float(obj.get("rotation", 0.0)) % 360.0)
    ry = math.radians(float(obj.get("rotation_y", 0.0)) % 360.0)
    rx = math.radians(float(obj.get("rotation_x", 0.0)) % 360.0)
    cos_rx, sin_rx = math.cos(rx), math.sin(rx)
    cos_ry, sin_ry = math.cos(ry), math.sin(ry)
    cos_rz, sin_rz = math.cos(rz), math.sin(rz)
    y1 = local_y * cos_rx - local_z * sin_rx
    z1 = local_y * sin_rx + local_z * cos_rx
    x2 = local_x * cos_ry + z1 * sin_ry
    z2 = -local_x * sin_ry + z1 * cos_ry
    x3 = x2 * cos_rz - y1 * sin_rz
    y3 = x2 * sin_rz + y1 * cos_rz
    return x3, y3, z2


def _runtime_tilted_box_top_height(obj, world_x, world_y):
    if obj.get("type") not in {"wall", "box"}:
        return None
    if abs(float(obj.get("rotation_x", 0.0))) < 0.01 and abs(float(obj.get("rotation_y", 0.0))) < 0.01:
        return None
    half_x = 0.5 * float(obj.get("scale_x", 1.0))
    half_y = 0.5 * float(obj.get("scale_y", 1.0))
    half_z = 0.5 * float(obj.get("height", 1.0))
    center_x = obj["center_x"]
    center_y = obj["center_y"]
    center_z = obj["base_z"] + half_z
    top_dx, top_dy, top_dz = _rotate_runtime_local_point(0.0, 0.0, half_z, obj)
    axis_u = _rotate_runtime_local_point(1.0, 0.0, 0.0, obj)
    axis_v = _rotate_runtime_local_point(0.0, 1.0, 0.0, obj)
    origin_x = center_x + top_dx
    origin_y = center_y + top_dy
    origin_z = center_z + top_dz
    det = axis_u[0] * axis_v[1] - axis_u[1] * axis_v[0]
    if abs(det) < 1e-6:
        return None
    rel_x = world_x - origin_x
    rel_y = world_y - origin_y
    local_u = (rel_x * axis_v[1] - rel_y * axis_v[0]) / det
    local_v = (axis_u[0] * rel_y - axis_u[1] * rel_x) / det
    epsilon = 0.03
    if not (-half_x - epsilon <= local_u <= half_x + epsilon and -half_y - epsilon <= local_v <= half_y + epsilon):
        return None
    return origin_z + local_u * axis_u[2] + local_v * axis_v[2]


def get_cell_rotation(x, y, z_hint=None):
    runtime = _runtime_layers_at(x, y)
    if runtime is None:
        return 0.0
    layers, cell_x, cell_y = runtime
    chosen_rotation = 0.0
    for layer_index, layer in enumerate(layers):
        cell = layer[cell_y][cell_x]
        if cell["tile"] == "empty":
            continue
        layer_z = layer_index * 1.0
        if z_hint is None or layer_z <= z_hint + 0.05:
            chosen_rotation = float(cell.get("rotation", 0.0)) % 360.0
...
def is_wall(x, y, z=None):
    ...
    if runtime is not None:
        layers, cell_x, cell_y = runtime
        if z is None:
            return any(layer[cell_y][cell_x]["tile"] == "wall" for layer in layers) or bool(_nearby_runtime_collision_objects(x, y))
        for obj in _nearby_runtime_collision_objects(x, y):
            local_x, local_y, local_z = _inverse_rotate_runtime_point(x, y, z, obj)
            half_x = 0.5 * float(obj.get("scale_x", 1.0))
            half_y = 0.5 * float(obj.get("scale_y", 1.0))
            half_z = 0.5 * float(obj.get("height", 1.0))
            if not (-half_x <= local_x <= half_x and -half_y <= local_y <= half_y):
            ...
            if obj["type"] == "stair":
                normalized_x = (local_x + half_x) / max(1e-6, half_x * 2.0)
                normalized_y = (local_y + half_y) / max(1e-6, half_y * 2.0)
                stair_floor = obj["base_z"] + _ramp_progress(normalized_x, normalized_y, float(obj.get("rotation", 0.0))) * obj["height"]
                if z < stair_floor - 0.18:
                    return True
                continue
            tilted_top = _runtime_tilted_box_top_height(obj, x, y)
            if tilted_top is not None:
                if z < tilted_top - 0.18:
                    return True
                continue
            if -half_z <= local_z <= half_z:
                return True
        link_height = _stair_link_height(x, y, z_hint=z)
        if link_height is not None and z < link_height - 0.18:
            return True
        return False
    if x < 0 or y < 0:
        return True
...
def get_floor_height(x, y, z_hint=None):
    runtime = _runtime_layers_at(x, y)
    if runtime is not None:
        layers, cell_x, cell_y = runtime
        floor_z = 0.0
        max_allowed = float("inf") if z_hint is None else z_hint + RAMP_REACH_HEIGHT
        for layer_index, layer in enumerate(layers):
            cell = layer[cell_y][cell_x]
            if cell["tile"] == "wall" or not cell.get("has_floor", True):
                continue
            candidate = layer_index * 1.0
            if cell["tile"] == "stair":
                stair_height = cell["height"] * 1.0
                candidate = _stair_floor_height(x, y, layer_index, cell)
                stair_bottom = layer_index * 1.0
                stair_top = stair_bottom + stair_height
                on_same_stair = z_hint is not None and stair_bottom - 0.22 <= z_hint <= stair_top + 0.35
                if (candidate <= max_allowed or on_same_stair) and candidate >= floor_z:
                    floor_z = candidate
                continue
            if candidate <= max_allowed and candidate >= floor_z:
                floor_z = candidate
        link_height = _stair_link_height(x, y, z_hint=z_hint)
        if link_height is not None and link_height >= floor_z:
            floor_z = link_height
        for obj in _nearby_runtime_collision_objects(x, y):
            candidate = None
            if obj["type"] == "stair":
...
def get_ceiling_height(x, y, z_hint=None):
    runtime = _runtime_layers_at(x, y)
    if runtime is not None:
        layers, cell_x, cell_y = runtime
        current_floor = get_floor_height(x, y, z_hint=z_hint)
        best = None
        for layer_index, layer in enumerate(layers):
            cell = layer[cell_y][cell_x]
            layer_floor = layer_index * 1.0
            if cell.get("has_floor", True) and layer_floor > current_floor + 0.001:
                best = layer_floor
                break
            ceiling_z = layer_floor + CUSTOM_ROOM_CEILING_HEIGHT
            if cell.get("has_ceiling", True) and ceiling_z > current_floor + 0.001:
                best = ceiling_z
                break
        for obj in _nearby_runtime_collision_objects(x, y):
            ...
            if span is None:
                continue
            bottom_z, _top_z = span
            if bottom_z <= current_floor + 0.001:
                continue
            if best is None or bottom_z < best:
                best = bottom_z
        if best is not None:
            return best
        return max(1.3, current_floor + 2.4)
    if x < 0 or y < 0:
        return 1.3
    if int(y) >= len(CEILING_MAP):
        return 1.3
    if int(x) >= len(CEILING_MAP[0]):
        return 1.3
    cell = CEILING_MAP[int(y)][int(x)]
    if cell == "4":
        return 3.0
    if cell == "3":
        return 2.4
...
def has_upper_wall(x, y):
    runtime = _runtime_layers_at(x, y)
    if runtime is not None:
        layers, cell_x, cell_y = runtime
        for layer_index, layer in enumerate(layers):
            cell = layer[cell_y][cell_x]
            if cell["tile"] == "wall" and (layer_index > 0 or cell["height"] > 1):
                return True
        return False
    if x < 0 or y < 0:
        return False
    if int(y) >= len(UPPER_WALL_MAP):
        return False
    if int(x) >= len(UPPER_WALL_MAP[0]):
        return False
    return UPPER_WALL_MAP[int(y)][int(x)] == "#"
...
def start_tutor_maze(root=None):
    ...
    player_spawn_x, player_spawn_y = _find_player_spawn()
    ...
    def restore_deja_vu_snapshot(snapshot):
        nonlocal player_x, player_y, player_z, vertical_velocity, player_angle, player_health, player_restart_at
    ...
    while running:
        ...
        if not elevator_active and not stats_window_active and not start_cutscene_active:
            ...
            def can_walk_to(x_pos, y_pos):
                can_stand, target_ground = can_occupy_position(
                    ...
                    body_height=DEFAULT_EYE_HEIGHT,
                )
        ...
        if not elevator_active and not stats_window_active and not start_cutscene_active:
            ...
            if player_z <= ground_z or (vertical_velocity <= 0.0 and abs(player_z - ground_z) <= GROUND_SNAP_DISTANCE):
                player_z = ground_z
                vertical_velocity = 0.0
            if player_z + DEFAULT_EYE_HEIGHT >= ceiling_z:
                player_z = max(ground_z, ceiling_z - DEFAULT_EYE_HEIGHT)
        ...
        depth_buffer = ray_engine.raycast_walls(
            ...
            ceiling_height_getter=get_ceiling_height,
        )
