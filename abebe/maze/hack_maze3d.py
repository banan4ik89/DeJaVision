...
from abebe.maze.raycast_engine import (
    NUM_RAYS,
...
from abebe.core.user_settings import get_fov_radians, get_game_view_size, get_show_debug_stats, get_show_fps, get_view_bob
...
def start_hack_maze(root, hack_window=None, on_success=lambda: None):
    ...
    while running:
        ...
        if pending_action == "secret" and time.time() >= pending_until:
            from abebe.maze.secret_maze import start_secret_maze
        ...
        if dialog_active:
            ...
            lines = (dialog_display_text + (" в–€" if dialog_typing else "")).split("\n")
            yy = int(H * 0.68)
            for line in lines:
