...
def draw_floor_ceiling(surface, W, H, num_steps=60, ceiling_base=120, floor_base=120, player_pitch=0):
    horizon = int(H // 2 + player_pitch)
...
class RaycastEngine:
    def __init__(self, screen, W, H, wall_tex, tex_size=32):
        ...
        self.last_player_pitch = 0.0
        self.last_eye_height = DEFAULT_EYE_HEIGHT

    def _project_world_y(self, world_z, depth, player_z, bob_offset, player_pitch, eye_height):
        eye_z = player_z + eye_height
        return int(self.H / 2 + bob_offset + player_pitch - ((world_z - eye_z) / max(depth, 0.05)) * self.H)
    ...
    def raycast_walls(
        ...
        self.last_player_pitch = player_pitch
    ...
    def render_sprite(
        ...
        W = self.W
        H = self.H
        if player_pitch is None:
            player_pitch = self.last_player_pitch
